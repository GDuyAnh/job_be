...
export class SearchJobDto {
  ...
  @ApiPropertyOptional({ description: 'Type of employment (array of number IDs)' })
  @IsOptional()
  @IsArray()
  @Transform(({ value }) =>
    Array.isArray(value) ? value.map(Number) : [Number(value)]
  )
  @IsNumber({}, { each: true, message: 'Each type of employment must be a number' })
  typeOfEmployment?: number[];

  @ApiPropertyOptional({ description: 'Experience level (array of number IDs)' })
  @IsOptional()
  @IsArray()
  @Transform(({ value }) =>
    Array.isArray(value) ? value.map(Number) : [Number(value)]
  )
  @IsNumber({}, { each: true, message: 'Each experience level must be a number' })
